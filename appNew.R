...
ui <- material_page(
  ...
  material_tab_content(
    ...
    HTML('<center><h4><b>Welcome to Linh Davis Real Estate</b></h4></center>'),
    material_row(
      material_column(width = 12, align = 'center',
        HTML('<center><p>My goal is to assist Houstonians achieve their real estate goals.  
            As your family prepares to make one of the biggest purchases of your life,
            I am here to help you find your perfect home, while providing the analytic tools to 
            create a true advantage in your negotiations.<br><br>I am committed to cultivating community and creating an 
            exceptional experience among clients, peers, and service providers.</p></center>')
      )
    ),
    br(),
    material_row(
      ...
      material_column(width = 12, offset = 0, align = 'center',
                      material_modal(
                        ...
                        material_card(
                          title = '',
                          depth = 2,
                          HTML('<center><a><img src="ld.jpg"  width = "55%"></a></center>'),
                          p('Texas is in my roots.  I grew up southeast of Houston in Palacios and have lived in the city 
                            since graduating from Texas A&M in 2011.  I know and love the city and am ready to bring this 
                            passion with us as we find your next home.'),
                          br(),
                          h4('Contact Information'),
                          h6(strong('Phone:')),
                          p('(832) 764-1737'),
                          h6(strong('Email:')),
                          p('linhdavishomes@gmail.com'),
                          br(),
                          HTML('<a href="https://www.facebook.com/linhdavisrealtor" target = "_blank">
  ...
  material_tab_content(
   ...
   material_row(
     material_column(width = 12, align = 'center',
       ...
       email_input('email',label = tagList(icon("envelope"), "Email")),
       email_input('phone',label = tagList(icon("phone"),"Phone")),
       
       shinymaterial::material_button('push', label = 'Access Houston Market Analytics', depth = 3, color = 'red darken-4')
     )
   ),
    material_row(
      material_column(width = 3,# align = 'center',
        material_card(
          ...
          material_card(
            ...
            #uiOutput('homes'),
            #depth = 3,
            # material_slider('rooms', h5('Bedrooms'), min_value = 0, max_value = 6, step_size = 0.5, initial_value = c(2,4) ),
            # 
            # 
...
server <- function(input, output, session){
  ...
  observeEvent(input$push, {
    if(input$email == ''|input$phone == ''){
      ...
      dfx <- data.frame(email = input$email, phone = input$phone)
      dfx1 <- readRDS('./data/users.rds')
      dfx1 <- dfx1 %>% rbind(dfx) %>% distinct()
      saveRDS(dfx1, './data/users.rds')
